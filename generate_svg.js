...
async function buildSVG() {
  ...
  if (!fs.existsSync(imgPath)) {
    console.error("logo.png not found here!");
  }

  // Resize using Jimp to drastically reduce base64 size (GitHub Amoeba bypass)
  const image = await Jimp.read(imgPath);
  image.resize(150, 150); // Small size, scaled up via SVG
  ...
  const svgContent = `<svg width="450" height="450" viewBox="0 0 450 450" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
    <style>
      ...
      @keyframes modernFloat {
        0% { transform: translateY(0px) scale(1); filter: drop-shadow(0 0 10px rgba(0, 210, 255, 0.6)) drop-shadow(0 0 20px rgba(58, 123, 213, 0.4)); }
        50% { transform: translateY(-20px) scale(1.05); filter: drop-shadow(0 0 25px rgba(0, 255, 255, 1)) drop-shadow(0 0 45px rgba(0, 150, 255, 0.8)); }
        100% { transform: translateY(0px) scale(1); filter: drop-shadow(0 0 10px rgba(0, 210, 255, 0.6)) drop-shadow(0 0 20px rgba(58, 123, 213, 0.4)); }
      }
      .animated-logo {
  ...
  fs.writeFileSync('logo_animated.svg', svgContent);
  console.log('logo_animated.svg created successfully!');
}
